...
export default function Reports() {
  ...
  const downloadAttendanceExcel = () => {
    ...
    downloadBlob(
      `${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/reports/attendance/excel?branchId=${branchId}&from=${from}&to=${to}`,
  ...
  const downloadMonthlySummary = () => {
    if (!branchId) { alert('Şube seçin'); return }
    downloadBlob(
      `${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/reports/monthly-summary/excel?branchId=${branchId}&year=${year}&month=${month}`,
  ...
  const downloadLeavesExcel = () => {
    downloadBlob(
      `${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/reports/leaves/excel?from=${from}&to=${to}`,
  ...
  const loadPreview = () => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/attendance/daily?branchId=${branchId}&date=${to}`, { headers: headers() })
      .then(r => r.json())
  ...
  const loadLateStats = () => {
    if (!branchId) return
    setPreviewLoading(true)
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/reports/late-stats?branchId=${branchId}&from=${from}&to=${to}`, { headers: headers() })
      .then(r => r.json())
