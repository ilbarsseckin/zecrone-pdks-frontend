...
export default function Employees() {
  ...
  const load = () => {
    Promise.all([
      fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/employees', { headers: authHdr() }).then(r => r.json()),
      fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/branches',  { headers: authHdr() }).then(r => r.json()),
    ]).then(([emps, brs]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const res = await fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/employees', {
      method: 'POST',
  ...
  const setPassive = async (id: string) => {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/employees/${id}/status?status=PASSIVE`, {
  ...
  const setMobilePass = async () => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/employees/${mobilePassModal.id}/set-mobile-password?password=${encodeURIComponent(mobilePassword)}`,
