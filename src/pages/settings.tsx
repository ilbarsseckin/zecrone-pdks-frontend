...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/tenants/me', { headers: headers() })
      .then(r => r.json())
      .then(data => {
        setProfile({
  ...
  const saveProfile = async (e: React.FormEvent) => {
    e.preventDefault()
    setError('')
    const res = await fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/tenants/me', {
      method: 'PATCH',
  ...
  const savePassword = async (e: React.FormEvent) => {
    ...
    const res = await fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/auth/change-password', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...headers() },
      body: JSON.stringify({
