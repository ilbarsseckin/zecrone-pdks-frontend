...
export default function SuperAdminDashboard() {
  ...
  const load = () => {
    Promise.all([
      fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/superadmin/stats',   { headers: headers() }).then(r => r.json()),
      fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/superadmin/tenants', { headers: headers() }).then(r => r.json()),
    ]).then(([s, t]) => {
      setStats(s)
  ...
  const loadAdmins = () => {
    fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/superadmin/admins', { headers: headers() })
      .then(r => r.json()).then(data => setAdmins(Array.isArray(data) ? data : []))
  }

  const toggleActive = async (id: string, isActive: boolean) => {
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/superadmin/tenants/${id}/${isActive ? 'deactivate' : 'activate'}`, {
  ...
  const changePlan = async () => {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/superadmin/tenants/${planModal.id}/plan?plan=${selectedPlan}`, {
      method: 'PATCH', headers: headers()
    })
  ...
  const saveSubscription = async () => {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/superadmin/tenants/${subModal.id}/subscription?${params}`, {
      method: 'PATCH', headers: headers()
    })
  ...
  const createAdmin = async () => {
    ...
    const res = await fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/superadmin/admins', {
      method: 'POST',
  ...
  const deleteAdmin = async (id: string) => {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/superadmin/admins/${id}`, {
