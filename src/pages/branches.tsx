...
export default function Branches() {
  ...
  const load = () => {
    fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/branches', {
  ...
  const handleNewBranch = () => {
    if (features.maxBranches !== Infinity && branches.length >= features.maxBranches) {
      alert(`Planınız en fazla ${features.maxBranches} şubeye izin veriyor.`)
      return
    }
    setShowModal(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const res = await fetch('h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/branches', {
      method: 'POST',
  ...
  const deleteBranch = async (id: string) => {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'h${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}'}/api/branches/${id}`, {
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg md:text-xl font-bold text-gray-800 dark:text-gray-100">Şubeler</h2>
          ...
          {features.maxBranches !== Infinity && branches.length >= features.maxBranches && (
            <div className="mb-4 flex items-center justify-between bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl px-4 py-3 gap-3">
              <div className="flex items-center gap-2">
                <span>⚠️</span>
                <span className="text-xs md:text-sm text-amber-800 dark:text-amber-400">
                  Şube limitine ulaştınız. Planı yükseltin.
                </span>
              </div>
              <a href="/plan-upgrades" className="text-xs font-semibold text-blue-600 whitespace-nowrap">Yükselt →</a>
            </div>
          )}

          {/* Mobilde 1 kolon, tablette 2, masaüstünde 3 */}
              ...
              <div key={branch.id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-xs">{branch.timezone}</span>
                  </div>
                  {branch.phone && (
                    <div className="flex justify-between">
                      <span>Telefon</span>
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                  <button onClick={() => deleteBranch(branch.id)}
                    className="w-full py-2 text-xs text-red-500 border border-red-100 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20">
                    Sil
                  </button>
                </div>
              </div>
            ...
            <div className="fixed inset-0 bg-black/50 flex items-end md:items-center justify-center z-50 p-4">
              <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-5">
                  <h3 className="font-bold text-lg">Yeni Şube</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-medium mb-1">Tolerans (dk)</label>
